...
// 금액 셀 검사
function highlightAmtCells(node) {
...
const observerAmt = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) {
                // 새로 추가된 노드가 행(tr) 또는 셀일 경우 검사
                if (node.matches('tr') || node.querySelector('[data-item-key="amt_salesXlist"]')) {
                    highlightAmtCells(node);
                }
            }
        });
    });
...
observerAmt.observe(document.body, { childList: true, subtree: true });